...
const Results = (props) => {
  ...
  React.useEffect(async () => {
    ...
    ids.forEach(async (id) => {
      ...
      setTestData((testData) => [...testData, details]);
    });
  }, []);
          ...
          {testData &&
            testData.map((ele) => (
              <div tw="bg-primary-500 rounded-lg p-20">
                <Container>
                  <h2 tw="text-base sm:text-lg font-bold">{ele.id}</h2>
                  {ele.questions.map((q, i) => {
                    <h4 tw="text-base sm:text-lg font-bold">
                      {i} : {q.given}
                    </h4>;
                  })}
                </Container>
